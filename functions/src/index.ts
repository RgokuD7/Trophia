...
// Interface definitions (mirroring types.ts)
interface UserProfile {
  name: string;
  takesCreatine?: boolean;
  lastCreatineIntake?: string;
...
export const checkCreatineMorning = onSchedule({
  schedule: "0 9 * * *",
...
export const checkCreatineAndFoodLunch = onSchedule({
  schedule: "0 14 * * *",
  timeZone: "America/Santiago",
  memory: "256MiB"
}, async (event) => {
  ...
  for (const doc of usersSnap.docs) {
    const user = doc.data() as UserProfile;
    if (!user.isOnboardingCompleted) continue;
...
export const checkHydrationAfternoon = onSchedule({
  schedule: "0 18 * * *",
  timeZone: "America/Santiago",
  memory: "256MiB"
}, async (event) => {
...
export const checkDeficitNight = onSchedule({
  schedule: "30 21 * * *",
  timeZone: "America/Santiago",
  memory: "256MiB"
}, async (event) => {
  ...
  for (const doc of usersSnap.docs) {
    ...
    const loggedSportsToday = (user as any).loggedSportsToday || [];
    const todaySports = loggedSportsToday.filter((s: any) => s.date === todayStr);
    const sportsCalories = todaySports.reduce((sum: number, s: any) => sum + (s.caloriesBurned || 0), 0);
